...
export default function InventarioPage() {
  ...
  async function guardarArticulo(values) {
    try {
      ...
      if (editing) {
        await updateArticulo(editing.id, payload);
        ...
        const articulo = await createArticulo(payload);
        ...
        <tbody>
          {articulos?.map((articulo) => (
            <tr key={articulo.id}>
              ...
              <td>{articulo.tallas?.nombre}</td>
              <td>{articulo.propietarios?.nombre}</td>
              <td>
                {articulo.stock_actual}
                {articulo.stock_actual === 0 && <div>⚠ Agotado</div>}
              ...
              <td>
                <button onClick={() => setEditing(articulo)}>
                  Editar
                </button>
                ...
                {articulo.activo ? (
                  ...
                  <button
                    onClick={async () => {
                      await activarArticulo(articulo.id);
                      refetch();
                    }}
                  >
                    Activar
